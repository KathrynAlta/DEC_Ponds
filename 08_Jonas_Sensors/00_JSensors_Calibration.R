...
list.files(pattern = ".csv", full.names = T) %>%  #Names of all the file in the folder that end in .csv
  ...
  filter(datetime > ymd_hms("2023-07-01 00:00:00"),  #filter any data not from the day you are interested in 
...
setwd("~/K Gannon/Data_Offload/JSensors/Calibration_Data/230809_JSensor_Calib") #Reset the working directory to the folder containing just the files that need to be formatted
raw_file_names <- list.files(pattern="*.csv") #Get a list of all of the .txt files in the working directory 
list_of_raw_Jsensor <- lapply(raw_file_names, read.table, header = T, sep = ",", fill = TRUE)
jsensor <- do.call("rbind", list_of_raw_Jsensor)
str(jsensor)
jsensor$datetime <- as.POSIXct(jsensor$datetime,tz = "UTC", format = "%Y/%m/%d %H:%M:%OS")


# Format the long data 
jsensor %>% 
  filter(datetime > ymd_hms("2023-08-08 00:00:00"),  #filter any data not from the day you are interested in 
         CH4smV < 1000 ) %>%  #remove spillover (values above 1000)
  mutate(datetime = case_when(sensor == "J1" ~ datetime + 3283,  #Times are off so add and subtract to align based on CH4 peaks
                              sensor == "J4" ~ datetime - 8,
                              T ~ datetime)) -> raw_data  
